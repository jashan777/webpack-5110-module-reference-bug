...
for V in 5.109.2 5.110.2; do
  ...
  USE_FIX=0 CONCATENATE=1     build; report "whole-module require()"
  USE_FIX=1 CONCATENATE=1     build; report "  + fix at require site"
  USE_FIX=0 CONCATENATE=nocjs build; report "  + concatenateModules{commonjs:0}"
  USE_FIX=0 CONCATENATE=0     build; report "  + concatenateModules:false"
done

echo; rule; printf "  ${B}What actually shipped (webpack 5.110.2)${X}\n"; rule
USE_FIX=0 CONCATENATE=1 build
